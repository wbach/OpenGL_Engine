...
int bmp_generator(const std::string & filename, int width, int height, const unsigned char * data)
{
	...
	// copy the data
	FILE *fp;
	if (!(fp = fopen(const_cast<char*>(filename.c_str()), "wb"))) return 0;
...
{
	FILE *fp;
	if (!(fp = fopen(const_cast<char*>(filename.c_str()), "wb"))) return;
	fwrite(&data[0], sizeof(unsigned char), data.size(), fp);
	fclose(fp);
}
