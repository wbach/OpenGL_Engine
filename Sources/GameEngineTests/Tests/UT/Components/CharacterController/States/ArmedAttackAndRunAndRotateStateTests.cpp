...
void prepareState(CharacterControllerTests& test)
{
    ...
    test.tiggerAndExpect<RotateLeftEvent>({test.sut_.animationClipsNames_.armed.run.forward});
...
{
    prepareState(*this);
    tiggerAndExpect<EndAttackEvent>({sut_.animationClipsNames_.armed.run.forward});
}
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunAndRotateState_EndRotationEvent)
{
    ...
    expectNoRotation();
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    tiggerAndExpect<EndRotationEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
}
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunAndRotateState_WeaponStateEvent)
{
    prepareState(*this);
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    tiggerAndExpect<GameEngine::WeaponStateEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunAndRotateState_DrawArrowEvent)
{
    prepareState(*this);
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    tiggerAndExpect<GameEngine::DrawArrowEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunAndRotateState_AimStopEvent)
{
    ...
    tiggerAndExpect<GameEngine::DrawArrowEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
    expectAnyRotation();
    tiggerAndExpect<GameEngine::AimStopEvent>({clipName, sut_.animationClipsNames_.armed.run.forward}, {0});
