...
void prepareState(CharacterControllerTests& test)
{
    ...
    test.tiggerAndExpect<CrouchChangeStateEvent>(
        {test.sut_.animationClipsNames_.drawArrow, test.sut_.animationClipsNames_.armed.crouch.forward});
}
}  // namespace
...
TEST_F(CharacterControllerTests, DrawArrowCrouchWalkState_WeaponStateEvent)
{
    ...
    tiggerAndExpect<WeaponStateEvent>(
        {sut_.animationClipsNames_.disarm, sut_.animationClipsNames_.disarmed.crouch.forward});
...
TEST_F(CharacterControllerTests, DrawArrowCrouchWalkState_MoveLeftEvent)
{
    ...
    tiggerAndExpect<MoveLeftEvent>({sut_.animationClipsNames_.drawArrow, sut_.animationClipsNames_.armed.crouch.forward});
}
...
{
    prepareState(*this);
    tiggerAndExpect<EndForwardMoveEvent>({sut_.animationClipsNames_.drawArrow, sut_.animationClipsNames_.armed.crouchIdle});
}
TEST_F(CharacterControllerTests, DrawArrowCrouchWalkState_EndBackwardMoveEvent)
{
    prepareState(*this);
    tiggerAndExpect<EndForwardMoveEvent>({sut_.animationClipsNames_.drawArrow, sut_.animationClipsNames_.armed.crouchIdle}, {0});
    expectForwardVelocity(-DEFAULT_BACKWARD_WALK_SPEED);
    tiggerAndExpect<MoveBackwardEvent>(
                {sut_.animationClipsNames_.drawArrow, sut_.animationClipsNames_.armed.crouch.backward});
    tiggerAndExpect<EndBackwardMoveEvent>({sut_.animationClipsNames_.drawArrow, sut_.animationClipsNames_.armed.crouchIdle}, {0});
}
