...
void prepareState(CharacterControllerTests& test)
{
    ...
    test.tiggerAndExpect<AttackEvent>(
        {test.sut_.animationClipsNames_.armed.attack.front().name, test.sut_.animationClipsNames_.armed.run.forward});
}
}  // namespace
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_MoveLeftEvent_EndMoveLeftEvent)
{
    ...
    tiggerAndExpect<MoveLeftEvent>({attackClipName, sut_.animationClipsNames_.armed.run.forward}, {0});
    expectRootboneRotation(VECTOR_FORWARD + VECTOR_LEFT);
    expectVelocity(VECTOR_LEFT, vec3(DEFAULT_RUN_SPEED, 0.0, 0.0));
    tiggerAndExpect<EndForwardMoveEvent>({attackClipName, sut_.animationClipsNames_.armed.run.forward}, {0});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_MoveRightEvent_EndMoveRightEvent)
{
    ...
    tiggerAndExpect<MoveRightEvent>({attackClipName, sut_.animationClipsNames_.armed.run.forward}, {0});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_WeaponStateEvent)
{
    prepareState(*this);
    const auto& attackClipName = sut_.animationClipsNames_.armed.attack.front().name;
    tiggerAndExpect<WeaponStateEvent>({sut_.animationClipsNames_.armed.run.forward, attackClipName});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_EndBackwardMoveEvent)
{
    ...
    expectForwardVelocity(DEFAULT_RUN_SPEED);
    tiggerAndExpect<EndBackwardMoveEvent>({clipName, sut_.animationClipsNames_.armed.run.forward}, {0});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_RotateLeftEvent)
{
    ...
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    expectRotationLeft();
    tiggerAndExpect<RotateLeftEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
}
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_RotateRightEvent)
{
    ...
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    expectRotationRight();
    tiggerAndExpect<RotateRightEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
}
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_RotateTargetEvent)
{
   ...
   tiggerAndExpect<RotateTargetEvent>(RotateTargetEvent{targetRotation.value_},
                                      {clipName, sut_.animationClipsNames_.armed.run.forward});
}
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_SprintStateChangeEvent)
{
    prepareState(*this);
    const auto& clipName = sut_.animationClipsNames_.armed.attack.front().name;
    tiggerAndExpect<SprintStateChangeEvent>({clipName, sut_.animationClipsNames_.armed.run.forward});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_MoveForwardAndLeftEvent)
{
    ...
    prepareState(*this);
    expectVelocity(VECTOR_FORWARD + VECTOR_LEFT, vec3(DEFAULT_RUN_SPEED, 0.0, DEFAULT_RUN_SPEED));
    tiggerAndExpect<MoveLeftEvent>({clip.name, sut_.animationClipsNames_.armed.run.forward});
    expectVelocity(VECTOR_LEFT, vec3(DEFAULT_RUN_SPEED, 0.0, 0.0));
    tiggerAndExpect<EndForwardMoveEvent>({clip.name, sut_.animationClipsNames_.armed.run.forward}, {0});
...
TEST_F(CharacterControllerTests, ArmedAttackAndRunState_PR_MoveStopAndGoDuringAttack)
{
    ...
    tiggerAndExpect<MoveForwardEvent>({attackClip.name, sut_.animationClipsNames_.armed.run.forward}, {0});
    Update(ADVANCED_TIME_CLIP_TIME);
    Update(ADVANCED_TIME_TRANSITION_TIME);
    tiggerAndExpect<EndForwardMoveEvent>({sut_.animationClipsNames_.armed.idle}, {0});
}
