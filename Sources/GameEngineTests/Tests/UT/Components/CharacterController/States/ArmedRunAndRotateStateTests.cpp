...
void prepareState(CharacterControllerTests& test)
{
    ...
    test.tiggerAndExpect<RotateRightEvent>({test.sut_.animationClipsNames_.armed.movement.run.forward});
}
}  // namespace

TEST_F(CharacterControllerTests, ArmedRunAndRotate_CrouchChangeStateEvent)
{
    prepareState(*this);
...
TEST_F(CharacterControllerTests, ArmedRunAndRotate_EndBackwardMoveEvent)
{
    ...
    tiggerAndExpect<EndBackwardMoveEvent>({sut_.animationClipsNames_.armed.posture.stand.rotate.right});
}
...
TEST_F(CharacterControllerTests, ArmedRunAndRotate_MoveForwardEvent)
{
    ...
    expectNoMove();
    tiggerAndExpect<MoveForwardEvent>({sut_.animationClipsNames_.armed.posture.stand.rotate.right});
    expectForwardVelocity(DEFAULT_RUN_SPEED);
    tiggerAndExpect<EndBackwardMoveEvent>({sut_.animationClipsNames_.armed.movement.run.forward});
    expectRotationRight(DEFAULT_MOVING_CHANGE_DIR_SPEED);
...
{
    prepareState(*this);
    tiggerAndExpect<EndRotationEvent>({sut_.animationClipsNames_.armed.movement.run.forward});
}
