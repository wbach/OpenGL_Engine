...
CFont::~CFont()
{
    if (!isInit)
        return;
    DeleteQuad(quadVao, quadIndices, quadVertex, quadTexCoord);
...
void CFont::Init(const std::string& file_name_relative)
{
    ...
    if (!tryfile.is_open())
    {
        DEBUG_LOG("[Error] The file " + file_name + " wasnt successfuly opened \n");
        return;
    ...
    CreateQuad(quadVao, quadIndices, quadVertex, quadTexCoord, quadIndicesSize);
    ...
    for (uint32 j = 0; j < height; j++)
    {
        for (uint32 i = 0; i < width; i++)
        {
            expanded_data[2 * (i + j * width)] = expanded_data[2 * (i + j * width) + 1] =
                (i >= (uint32)bitmap.width || j >= (uint32)bitmap.rows) ? 0 : bitmap.buffer[i + bitmap.width * j];
        }
    }
