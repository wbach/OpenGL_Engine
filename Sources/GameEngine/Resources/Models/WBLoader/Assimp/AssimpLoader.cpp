...
{
    uint32 flags = aiProcess_Triangulate | aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals |
...
bool AssimpLoader::CheckExtension(const File& file)
{
    return file.IsExtension({"AMF", "3DS",     "AC",  "ASE",  "ASSBIN", "B3D",     "BVH", "COLLADA", "DXF", "CSM",
                             "HMP", "IRRMESH", "IRR", "LWO",  "LWS",    "MD2",     "MD3", "MD5",     "MDC", "MDL",
                             "NFF", "NDO",     "OFF", "OBJ",  "OGRE",   "OPENGEX", "PLY", "MS3D",    "COB", "BLEND",
                             "IFC", "XGL",     "FBX", "Q3D",  "Q3BSP",  "RAW",     "SIB", "SMD",     "STL", "TERRAGEN",
                             "3D",  "X",       "X3D", "GLTF", "3MF",    "MMD",     "STEP"});
...
void CAssimModel::InitModel(const std::string& file_name)
{
    ...
    if (scene)
    {
        ...
        {
            std::string error =
                "[Error] The file " + filename + " wasnt successfuly opened - AI_SCENE_FLAGS_INCOMPLETE";
            throw std::runtime_error(error.c_str());
        }
...
void CAssimModel::ReadCollisions(std::string file_name, std::vector<float>& postions, std::vector<float>& normals,
                                 std::vector<uint32>& indices)
{
    ...
    if (scene)
    {
        ...
        {
            std::string error =
                "[Error] The file " + file_name + " wasnt successfuly opened - AI_SCENE_FLAGS_INCOMPLETE";
            throw std::runtime_error(error.c_str());
        }
...
void CAssimModel::RecursiveProcess(const aiScene* scene, aiNode* node, std::vector<float>& postions,
                                   std::vector<float>& normals, std::vector<uint32>& indices)
{
    // proces
    for (uint32 i = 0; i < node->mNumMeshes; i++)
    {
...
{
    mat4 m;
    m[0][0] = assimp_atrix.a1;
    m[0][1] = assimp_atrix.b1;
    m[0][2] = assimp_atrix.c1;
    m[0][3] = assimp_atrix.d1;
    m[1][0] = assimp_atrix.a2;
    m[1][1] = assimp_atrix.b2;
    m[1][2] = assimp_atrix.c2;
    m[1][3] = assimp_atrix.d2;
    m[2][0] = assimp_atrix.a3;
    m[2][1] = assimp_atrix.b3;
    m[2][2] = assimp_atrix.c3;
    m[2][3] = assimp_atrix.d3;
    m[3][0] = assimp_atrix.a4;
    m[3][1] = assimp_atrix.b4;
    m[3][2] = assimp_atrix.c4;
    m[3][3] = assimp_atrix.d4;
    return m;
}
...
void CAssimModel::SetTime(const float& time)
{
    currentTime = time;
}
...
const std::vector<mat4>* CAssimModel::GetBonesTransforms(uint32 mesh_id)
{
    return &boneTransformMatrixes[mesh_id];
}

void CAssimModel::BoneTransform(uint32 num_bones, std::vector<SBoneInfo>& bone_info,
                                std::map<std::string, uint32>& bone_mapping, float TimeInSeconds,
                                std::vector<mat4>& Transforms)
{
    mat4 Identity(1.0f);

    float TicksPerSecond =
        (float)scene->mAnimations[0]->mTicksPerSecond != 0 ? (float)scene->mAnimations[0]->mTicksPerSecond : 25.0f;
    ...
    for (uint32 i = 0; i < num_bones; i++)
    {
        if (Transforms.size() < i + 1)
            Transforms.push_back(mat4(1.f));
        Transforms[i] = bone_info[i].finalTransformation;
    }
}

void CAssimModel::ReadNodeHeirarchy(std::map<std::string, uint32>& bone_mapping, std::vector<SBoneInfo>& bone_info,
                                    float AnimationTime, const aiNode* pNode, const mat4& ParentTransform)
{
    ...
    if (bone_mapping.find(NodeName) != bone_mapping.end())
    {
        ...
        bone_info[BoneIndex].finalTransformation =
            globalInverseTransform * GlobalTransformation * bone_info[BoneIndex].boneOffset;
    }
