...
namespace Utils
{
	...
	public:
		...
		XmlNodeWrapper& AddAtributte(const std::string& name, const std::string& value)
		{
			...
			node_->append_attribute(document_->allocate_attribute(document_->allocate_string(name.c_str()), document_->allocate_string(value.c_str())));
	...
	void WriteNode(XmlNode& node, XmlNodeWrapper& nodeWrapper)
	{
		auto child = nodeWrapper.AddChild(node.name_, node.value_);
