...
namespace PhysicsTestGame
{
	...
	int PhysicsScene::Initialize()
	{
		...
		bt.terrainShape = std::make_unique<btHeightfieldTerrainShape>(terrain->GetSize().x, terrain->GetSize().y, &data[0], Terrain::HEIGHT_FACTOR, MIN_HEIGHT, MAX_HEIGHT, 1, PHY_FLOAT, false);
