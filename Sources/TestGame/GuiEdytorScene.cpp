...
int GuiEdytorScene::Initialize()
{
    ...
    // guiTexture.texture                  =
    // resourceManager_->GetTextureLaoder().LoadTexture("GUI/character_select.jpg", false);
...
int GuiEdytorScene::Update(float deltaTime)
{
    ...
    if (attached)
    {
        ...
        DEBUG_LOG(std::to_string(mouseMove));
...
{
    if (t == GuiEditingType::Texture)
        return "Texture";
    if (t == GuiEditingType::Text)
        return "Text";
    return std::string();
}
