...
{
	UtlisNetwork::SendMessage(clientSocket, message);
}
...
bool SDLClientGetway::CheckIncomingMessage()
{
	...
	auto recv = UtlisNetwork::CheckIncomingMessage(m_IncomingMessages, clientSocket, start_complex);
...
void SDLClientGetway::CheckComplexMessage()
{
	...
	if (UtlisNetwork::CheckComplexMessage(m_IncomingMessages))
