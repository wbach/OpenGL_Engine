...
void SDLServerGetway::GetMessage()
{
	...
	for (auto& user : users)
	{
		...
		bool complex_recv;
		auto receivedByteCount = UtlisNetwork::CheckIncomingMessage(incoming_messages, user.socket, complex_recv);
...
{
	UtlisNetwork::SendMessage(user.socket, message);
}
...
void SDLServerGetway::SendMessage(uint32 user, const std::string & message)
{
	...
	UtlisNetwork::SendMessage(users[user].socket, message);
}
