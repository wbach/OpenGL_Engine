...
private:
	...
	virtual const std::vector<glm::mat4>* GetBonesTransforms(uint32 mesh_id) override;
	...
	void BoneTransform(uint32 num_bones, std::vector<SBoneInfo>& bone_info, std::map<std::string, uint32>& bone_mapping, float TimeInSeconds, std::vector<glm::mat4>& Transforms);
	void ReadNodeHeirarchy(std::map<std::string, uint32>& bone_mapping, std::vector<SBoneInfo>& bone_info, float AnimationTime, const aiNode* pNode, const glm::mat4& ParentTransform);
